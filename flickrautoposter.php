...
add_action('init', 'fap_setup');
function fap_setup() {
		if (!wp_next_scheduled('fap_cron_hook')) {
			wp_schedule_event(time()+300, 'fap', 'fap_cron_hook');
		}
}

add_filter('cron_schedules', 'fap_more_reccurences');
function fap_more_reccurences($recc) {
	$recc['fap'] = array('interval' => 300, 'display' => 'Flickr Auto Poster Updates');
	return $recc;
}

add_action('fap_cron_hook','fap_postFlickrPictures');
function fap_postFlickrPictures() {
	...
	if(get_option('fap_flickrusername') && get_option('fap_tags') && get_option('fap_wpuserid')) {
		...
		require_once($fap_settings['fap_path'].'/lib/phpflickr/phpFlickr.php');
